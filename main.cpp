...
int main() {
    ...
    add_noise(delay_received, sigma_noise(received, 0_db));
    ...
    auto frames = std::vector<decltype(sampling)>();
    {
        ...
        for (auto i = 0; i < sampling.values.size() - FRAME_SIZE; i += FRAME_STEP) {
        ...
        for (auto i = frames.size() * FRAME_STEP; i < sampling.values.size() - 256; i += 256) {
            frames.push_back({std::vector<sample_t>(FRAME_SIZE), fs, fs.duration_of<_time_t>(i)});
